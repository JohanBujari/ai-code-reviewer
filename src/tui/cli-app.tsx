...
const COMMON_VARS: VarDef[] = [
  ...
  { key: 'AI_PROVIDER', label: 'AI Provider', type: 'select', choices: ['openai', 'anthropic', 'azure-openai'] },
];

const PROVIDER_VARS: Record<string, VarDef[]> = {
  openai: [{ key: 'OPENAI_API_KEY', label: 'OpenAI API Key', type: 'secret' }],
  anthropic: [{ key: 'ANTHROPIC_API_KEY', label: 'Anthropic API Key', type: 'secret' }],
  'azure-openai': [
    { key: 'AZURE_OPENAI_ENDPOINT', label: 'Azure OpenAI Endpoint', type: 'text', hint: 'e.g. https://your-resource.openai.azure.com' },
    { key: 'AZURE_OPENAI_API_KEY', label: 'Azure OpenAI API Key', type: 'secret' },
    { key: 'AZURE_OPENAI_DEPLOYMENT', label: 'Azure OpenAI Deployment', type: 'text' },
  ],
};

const WATCH_VARS: VarDef[] = [
  { key: 'WATCH_REPOS', label: 'Repos to watch', type: 'text', hint: 'comma-separated, format: project/repoId/repoName' },
];
...
function detectMissingVars(command: 'watch' | 'review', answers: Record<string, string>): VarDef[] {
  ...
  if (provider) {
    for (const def of (PROVIDER_VARS[provider] ?? [])) { if (!lookup(def.key)) missing.push(def); }
  }
  if (command === 'watch') {
    for (const def of WATCH_VARS) { if (!lookup(def.key)) missing.push(def); }
  }
  return missing;
}
...
function buildConfigFromEnv(
  ...
  const provider = require('AI_PROVIDER') as 'openai' | 'anthropic' | 'azure-openai';

  let ai: AiConfig;
  switch (provider) {
    case 'openai':
      ai = { provider: 'openai', apiKey: require('OPENAI_API_KEY'), model: lookup('OPENAI_MODEL') };
      break;
    case 'anthropic':
      ai = { provider: 'anthropic', apiKey: require('ANTHROPIC_API_KEY'), model: lookup('ANTHROPIC_MODEL') };
      break;
    case 'azure-openai':
      ai = {
  ...
  const repos: WatchedRepo[] = reposRaw.split(',').map((entry) => {
    ...
    if (parts.length < 2) throw new Error(`Invalid WATCH_REPOS entry: "${entry}"`);
    return { project: parts[0], repoId: parts[1], repoName: parts[2] ?? parts[1] };
  });

  return { ...base, repos, pollIntervalMs: parseInt(options.interval ?? '30', 10) * 1000, stateFilePath: options.stateFile ?? './pr-agent-state.json' };
}
...
function CliApp({ initialCommand, reviewUrl, options }: CliAppProps) {
  ...
  const [command, setCommand] = useState<'watch' | 'review' | null>(initialCommand ?? null);
  ...
  const [prInfo, setPrInfo] = useState<{ prId: number; project: string; repo: string } | null>(null);
  // Handle q to exit on terminal phases
  useInput((input) => {
    if ((phase === 'review-done' || phase === 'review-error') && input === 'q') {
      exit();
    }
  });
  ...
  const launchWatch = useCallback((answers: Record<string, string>) => {
  ...
  const startReview = useCallback(async (prUrl: string, answers: Record<string, string> = {}) => {
    const match = prUrl.match(PR_URL_REGEX);
    if (!match) {
      setReviewError('Invalid Azure DevOps PR URL. Expected: https://dev.azure.com/{org}/{project}/_git/{repo}/pullrequest/{id}');
    ...
    try {
      ...
      const reviewer = new PrReviewer({ azureDevOps: config.azureDevOps, webhookSecret: '', ai: config.ai, logger });
  ...
  const handleConfigAnswer = useCallback((value: string) => {
          ...
          {missingVars[configIndex] && (
            <Box marginTop={configIndex > 0 ? 1 : 0}>
              {missingVars[configIndex].type === 'select' ? (
                <SelectInput
                  items={(missingVars[configIndex].choices ?? []).map((c) => ({ label: c, value: c }))}
